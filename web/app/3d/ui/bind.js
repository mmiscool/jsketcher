...
export function Object(dom, data, policy) {
}

function BindObject(dom, data, policy) {
  if (!policy) policy = DEFAULT_POLICY;
  const props = Object.getOwnPropertyNames(data);
  const scope = getScope(dom);
  for (let prop of props) {
    ...
    if (Array.isArray(value)) {
      bindFunc = BindArray;
      node = scope.nestedScopes[prop];
    } else if (typeof value === 'object') {
      bindFunc = BindObject;
      node = scope.nestedScopes[prop];
    } else {
      bindFunc = BindContent;
      node = scope.bindings[prop];
    }
    if (!node) continue;
    bindFunc(node, value, policy);
  }
  var callback = data[BINDING_CALLBACK];
  if (callback) {
    callback(dom, data, policy)
  }
}

function BindArray(node, array, policy, path) {
...
function clearScope(dom) {
  dom.data('BindingScope').removeData('BindingScope');
}
...
function index(dom) {
  ...
  while (queue.length != 0) {
    ...
    if (nestedScope) {
      ...
      scope.children().each((i, e) => queue.push($(e)));
    }
  }
  return scope;
}
...
function find(dom, selector) {
  return dom.find(selector).addBack(selector); // children and itself
}
...
function example(dom) {
  ...
  BindObject(dom, initState);
  //reordering, removing, updating provided attributes
  BindObject(dom, {users: [ {id:3}, {id:1, name: 'Peach-Beach'}, {id:2} ]});
  //only content update
  BindObject(dom, {users: {
